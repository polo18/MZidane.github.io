...
class PostsDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('action', function ($post) {
                $buttons = $this->button('posts.display', $post->slug, 'success', __('Show'), 'eye', '', '_blank');
                
                if (\Route::currentRouteName() === 'posts.indexnew') return $buttons;
               
                $buttons .= $this->button('posts.edit', $post->id, 'warning', __('Edit'), 'edit');

                if ($post->user_id === auth()->id()) {
                    $buttons .= $this->button('posts.create', $post->id, 'info', __('Clone'), 'clone');
                }

                return $buttons . $this->button('posts.destroy', $post->id, 'danger', __('Delete'), 'trash-alt', __('Really delete this post?'));
            })
    ...
    protected function getDate($post)
    {
        ...
        $html .= '<br>' . formatDate($updated ? $post->updated_at : $post->created_at) . __(' at ') . formatHour(
            $updated ? $post->updated_at : $post->created_at
        );
